...
for i in range(25000):
    for ip in ips:
        url = f"https://ubuntu.com/security/cves.json?limit=20&offset={i}"
        print(url)
        try:
                        ...
                        for b in range(len(information['packages'][a]['statuses'])):
                            ...
                            for x in records:
                                ...
                                if status == 'released' and status != 'upstream':
                                    ...
                                    values = (idcve, published, priority, package, distro, support, status, resolved_date)
                                    cursor.execute(sql, values)
                                    mydb.commit()
                                else:
                                    # Write to DataBase
                                    sql = "INSERT INTO ubuntu (CVE, Published, Priority, Package, Distro, Support, Status) VALUES (%s, %s, %s, %s, %s, %s, %s)"
                                    values = (idcve, published, priority, package, distro, support, status)
                                    cursor.execute(sql, values)
                                    mydb.commit()
