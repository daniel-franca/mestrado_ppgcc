...
def main(distro, version, sql):
    ...
    for x in records:
        cve=(str(x[0]))
        if x[1] is not None and "20" in x[1]:
            resolved = x[1]
            if distro == 'debian':
            ...
            if x[3] == "Affected" or x[3] == "Fix deferred" or x[3] == "Out of support scope" or x[1] == "CHECK MANUALLY":
...
for a in distro:
    if a == "debian":
        ...
        if a == "redhat":
            version = ["6", "7", "8", "9"]
            for b in version:
                sql = f"SELECT distinct (CVE), Resolved, Published_NIST, FixState FROM {a} where Version IN ('Red Hat Enterprise Linux {b}') and (F>                main(a, b, sql)
        else:
            if a == "ubuntu":
                version = ["xenial", "bionic", "focal", "jammy"]
                for b in version:
                    sql = f"SELECT distinct (CVE), Resolved, Published_NIST, Status FROM {a} where Distro IN ('{b}') AND Status != 'not-affected' A>                    main(a, b, sql)
            else:
                if a == "ubuntupro":
                    version = ["xenial", "bionic", "focal", "jammy"]
                    for b in version:
                        sql = f"SELECT distinct (CVE), Resolved, Published_NIST, Status FROM {a} where Distro IN ('{b}') AND Status != 'not-affecte>                        main(a, b, sql)
                else:
                    if a == "almalinux":
                        version = ["8", "9"]
                        for b in version:
                            sql = f"SELECT distinct (CVE), Resolved, Published_NIST, FixState FROM {a} where Version IN ('AlmaLinux {b}') and (FixS>                            main(a, b, sql)
                    else:
                        if a == "rockylinux":
                            version = ["8", "9"]
                            for b in version:
                                sql = f"SELECT distinct (CVE), Resolved, Published_NIST, FixState FROM {a} where Version IN ('Rocky Linux {b}') and>                                main(a, b, sql)
